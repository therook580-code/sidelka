...
def get_admin_channels(admin_id: int) -> list:
    return load_channels().get(str(admin_id), [])
...
class GiveawaySession:
    def __init__(self, prize, rules, duration_min, winners_count,
                 channel_id, channel_title, discussion_id, admin_id, admin_username):
        self.prize              = prize
        self.rules              = rules
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not channels:
        await update.message.reply_text(
            ...
            "Добавь канал командой /addchannel"
        )
    else:
        ch_list = "\n".join([f"• {ch.get('title', ch['channel_id'])}" for ch in channels])
        await update.message.reply_text(
            f"👋 Привет!\n\n"
            f"📢 Твои каналы:\n{ch_list}\n\n"
            f"📋 Команды:\n"
...
async def cmd_addchannel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📢 Добавление канала\n\n"
        "Введи @username канала (например @mychannel):\n\n"
        "❗️ Убедись что бот добавлен в канал и в группу обсуждений как администратор!",
...
async def addchannel_username(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if discussion_id:
        ...
        disc_text = (
            ...
            "Подключи группу к каналу в настройках Telegram,\n"
...
async def cmd_channels(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for i, ch in enumerate(channels, 1):
        disc = "✅" if ch.get("discussion_id") else "❌ нет обсуждений"
        active = " 🔴 ИДЁТ РОЗЫГРЫШ" if ch["channel_id"] in sessions else ""
        lines.append(f"{i}. {ch.get('title', ch['channel_id'])} ({ch['channel_id']})\n   {disc}{active}")
    await update.message.reply_text("📢 Твои каналы:\n\n" + "\n\n".join(lines) + "\n\n/addchannel — добавить ещё")
...
async def step_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not discussion_id:
        await update.message.reply_text(
            f"❌ У канала {ch.get('title', ch['channel_id'])} не подключена группа обсуждений!\n\n"
            f"Подключи группу и пересоздай канал через /addchannel",
            reply_markup=ReplyKeyboardRemove()
        )
        return ConversationHandler.END
    ...
    session = GiveawaySession(
        ...
        channel_title=ch.get("title", ch["channel_id"]),
        discussion_id=discussion_id,
        admin_id=user.id,
    ...
    await update.message.reply_text(
        f"✅ Пост опубликован в {ch.get('title', ch['channel_id'])}!\n\n"
        f"🎁 Приз: {d['prize']}\n"
        f"⏰ Время: {d['duration']} мин\n"
        f"🏆 Победителей: {d['winners']}\n\n"
        f"Результаты пришлю автоматически в личку.",
        reply_markup=ReplyKeyboardRemove()
    )
...
async def cmd_stop(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton(ch.get("title", ch["channel_id"]), callback_data=f"stop_ch:{ch['channel_id']}")]
                for ch in active]
    await update.message.reply_text("Какой розыгрыш остановить?", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def job_finish(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await ctx.bot.send_message(
            ...
            text=f"Розыгрыш в {session.channel_title} завершён!\n\n{result}",
            disable_web_page_preview=True
        )
    except Exception as e:
        log.error(f"Admin DM error: {e}")

    log.info(f"Победители: {[w['name'] for w in winners]}")
...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Следующий шаг — добавь свой канал:\n/addchannel"
    )
